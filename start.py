...
from Game_logic.data import player, list_monster, list_objects, list_weapon
...
def launch_screen():  
    ...
    while player_choice != True:
        ...
        if player_input == 1:
            ...
            start_game(player, list_monster, list_objects, list_weapon)
            
        elif player_input == 2:
            # load the save
            loaded_player, loaded_monsters, loaded_objects, loaded_weapon = load()
            if loaded_player:
                load_player = loaded_player
                load_list_monster = loaded_monsters
                load_list_objects = loaded_objects
                load_list_weapon = loaded_weapon
                start_game(load_player, load_list_monster, load_list_objects, load_list_weapon)
