...
from Game_logic.data import player, list_monster, list_objects
...
def launch_screen():  
    ...
    while player_choice != True:
        ...
        if player_input == 1:
            ...
            start_game(player, list_monster, list_objects)
            
        elif player_input == 2:
            # load the save
            loaded_player, loaded_monsters, loaded_objects = load()
            if loaded_player:
                load_player = loaded_player
                load_list_monster = loaded_monsters
                load_list_objects = loaded_objects
                start_game(load_player, load_list_monster, load_list_objects)
