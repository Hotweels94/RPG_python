...
# Verify if the player found a weapon        
...
# Verify if the player found an event    
...
# Verify if the player found a shop
def verify_shop_and_player_position(player, list_shop):
    for shop in list_shop:
        if player.position_x == shop.position_x and player.position_y == shop.position_y:
            shop.shop_sell(player)
            break 

# return random coordinates but it's impossible to have a forbidden coordinates
